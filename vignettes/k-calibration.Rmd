...
We can now call `calibrate()`, and give the values we want to reach as `target_values`.
```{r}
res_cal <- calibrate(
  res_mod,
...
Additional options to control the optimization process can be passed to `calibrate()`. These options are parameters of the  [optimx](https://CRAN.R-project.org/package=optimx) function, such as `upper` and `lower` to specify upper and lower values, `method` to change the optimization method, etc.
