...
To mitigate this drawback it is possible to limit the number of state expansion with `state_cycle_limit`. Because most time-varying values reach an asymptotic value quite fast, it is unnecessary to expand the states any further. The limit cycle value is repeated until the end.

The limit can be defined globally, per state, or per model and state.
