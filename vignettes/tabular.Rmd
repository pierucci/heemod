---
title: "Run models from tabular input"
date: "`r Sys.Date()`"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Run models from tabular input}
...
Most of the vignettes in `heemod` define models by `R` code. Models with large numbers of states and transition probabilities might be unwieldy to input at the keyboard, and could be conveniently specified by inputs in files.
...
The inputs for state definition, transition matrices, model options, etc. (referenced as *tabular inputs*) must be provided via `csv`, `xls` or `xlsx` files.^[French `csv` (with `;` separator) is not supported.] Columns starting by `.comment` are ignored, as well as blank rows.

These inputs must be provided in separate files. The path to these files is specified in a *reference file*, that acts as an 'umbrella' file atop of the other inputs.

*Warning for users of the `xls` format:*  
The `xls` (and `xlsx`) files are read with the `readxl` package. As of version 0.1.1 if a file contains strings and numeric values in one column the numeric values are rounded to 6 decimal places. We therefore recommend to use solely the `csv` or `xlsx` format.

```{r}
# res <- run_models_from_tabular(
#   "../inst/tabular/thr/",
#   "THR_specification.csv"
# )
```
...
```{r echo = FALSE}
heemod:::read_file("../inst/tabular/thr/THR_specification.csv") %>% 
  format_na %>% 
  knitr::kable()
```
...
A state file contains 2 mandatory columns: `.model` and `.state`, corresponding to model and state names respectively. The other columns correspond to the state values.
...
```{r echo = FALSE}
heemod:::read_file("../inst/tabular/thr/THR_states.csv") %>% 
  format_na %>% 
  knitr::kable()
```
...
```{r echo = FALSE}
heemod:::read_file("../inst/tabular/thr/THR_transition_probs.csv") %>% 
  format_na %>% 
  knitr::kable()
```
...
As with the state file above probabilities specified for only one model will be carried over to others. Only probabilities that differ from one model to another must be specified separately for each model. Unspecified transition probabilities are assumed to be 0.
...
```{r echo = FALSE}
heemod:::read_file("../inst/tabular/thr/THR_parameters.csv") %>% 
  format_na %>% 
  knitr::kable()
```
...
```{r echo = FALSE}
heemod:::read_file("../inst/tabular/thr/THR_options.csv") %>% 
  format_na %>% 
