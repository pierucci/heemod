...
For the example we define a simple model with only 1 strategy.   First we
use the partitioned survival framework:
...
In this case, we can define the same model using a Markov transition matrix.
Note that we will run the Markov model with `method = "end"` to match the
method used by partitioned survival models.
...
param <- define_parameters(
  ...
  p2 = compute_surv(
    km_medium %>% project(fitcov_medium, at = 730),
    time = model_time,
...
Note that we set method equal to "end" here, to match the method used by
partitioned survival model.
```{r fig.width=6, fig.height=4}
plot(resTM)
```

We can also define different models, for example, under the assumption
that state C can be reached only from state B:
```{r warning=FALSE, fig.width=6, fig.height=6}
