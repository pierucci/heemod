...
```{r}
cut_age <- function(x) {
  cut(x, c(35, 45, 55, 65, 75, 85, Inf), right = FALSE)
}

death_prob <- data.frame(
  age = cut_age(rep(seq(35, 85, 10), each = 2)),
  sex = rep(0:1, 6),
  rate = c(
...
param_standard <- define_parameters(
    ...
    mr = get_mortality_rate(cut_age(age), sex, death_prob)
)
param_standard
```
...
We also define optional `starting_values`, state values that are added to the total cost of all individuals once. This is especially useful to input the cost of an original intervention, here we use this parameter to specify the cost of the THR technology.
