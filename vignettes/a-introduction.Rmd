...
This document is an introduction to the basic steps to define and run a model in `heemod`. Note that even if this vignette uses the term *Markov model*, decision trees are actually a specific subset of such models, and thus can be specified easily with this package.
...
Now that the transition matrix and the state values are defined, we can combine them to create a Markov model with `define_strategy`:

```{r}
mod_1 <- define_strategy(
...
Time varying transition probabilities and state values are available through the `markov_cycle` and `state_cycle` variables.
