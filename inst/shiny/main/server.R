...
shinyServer(function(input, output, session) {
                       ...
                       lapply(1:nbStateVariables, function(i){
                         tags$tr(tags$td(variableStateName[i]),
                                 lapply (1:nbStates, function (j) {
                                   isolate({tags$td(textInput(paste0("stateVariable",x,i,j), value=ifelse(!is.null(input[[paste0("stateVariable",1,i,j)]]), input[[paste0("stateVariable",1,i,j)]],0), label=NULL, width="100%"))})
                                 }),
  ...
  output$nameStates <- renderUI({
    ...
    lapply(1:input$nbStates, function(i) {
      isolate({textInput(paste0("stateName", i), paste("State Name", i), value = ifelse(!is.null(input[[paste0("stateName",i)]]), input[[paste0("stateName",i)]], LETTERS[i]))})
    })
  })
  ...
  output$nameStrategies <- renderUI({
    ...
    lapply(1:input$nbStrategies, function(i) {
      isolate({textInput(paste0("strategyName", i), paste("Strategy Name", i), value = ifelse(!is.null(input[[paste0("strategyName",i)]]), input[[paste0("strategyName",i)]], as.character(as.roman(i))))})
    })
  })
