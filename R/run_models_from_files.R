...
#'
#' @param base_dir directory in which \code{ref-file} will be found.
#' @param ref_file name of a file specifying where model inputs will be found.
#' @param N.prob,cost,effect,init arguments to \code{\link{run_models}}
#'   and \code{\link{run_probabilistic}}
#' @param base_model which of the models should be the base model
#' @param save_outputs =TRUE should the outputs be saved? Only works if
#'   the reference file specifies an output directory.
#' @param overwrite = FALSE should the outputs be overwritten? Not relevant if 
#'   save_outputs = FALSE.
#' @param ... additional arguments passed to \code{\link{execute_models}}.
#' @return  an unevaluated models object
#' @details The reference file should have two columns, \code{data} 
#'   and \code{file}. 
#'   (Additional columns are allowed, but will be ignored.)
#'   "data" values must include "state", "tm", and "parameters", and also
#'   looks for "demographics" and "data".  The corresponding file values 
#'   give the names
#'   of the files (still in base_dir) that contain the corresponding
#'   information - or, in the case of "data", the directory containing data
#'   frames to be loaded. 
#'   
#'      For more information see the vignette: 
#'      \code{vignette("file-input", package = "heemod")}.`

#' @return a list with outputs models and model_runs (always),
#'   and, if appropriate input is provided, dsa (deterministic sensitivity
#'   analysis), psa (probabilistic sensitivity analysis) and demographics
...
#'   \item models (of type \code{uneval_model}, created by
#'   \code{\link{import_models_from_files}})
#'   \item param_info
#'   \item use_envir an environment containing
#'   data frames with information necessary for the models to run
#'   (see \code{\link{load_df_from_files}})
#'   \item output_dir where to store output files, if specified
#'   }
#' @details 
#'      For more information see the vignette: 
#'      \code{vignette("file-input", package = "heemod")}.`
...
gather_model_info <-
  function(base_dir, ref_file,  base_model = NULL) {
    models <- import_models_from_files(base_dir = base_dir,
                                       ref_file = ref_file,
    ...
    if("survivalDataDirectory" %in% ref$data){
      ...
      ## TODO - currently assumes fit_files and fit_names specified
      ##   separately, and in same order (that is, the first fit_name is
      ##   for the models fit using the first data, and so on, or,
      ##   alternately, that the first fit_file contains the model
      ##   referred to by the first fit_name).  Eventually probably
      ##   want to set this up in a way that's more error-proof.
      surv_inputs <- 
...
#' Execute models
#' @description Execute a full set of analyses, possibly including
#'   discrete sensitivity analysis, probabilistic sensitivity analysis,
#'   and analyses across demographics.
#'
#' @param inputs a list containing the unevaluated models ($models), 
#'   parameters for  the base model and sensitivity analyses ($param_info), 
#'   and possibly an environment containing additional information 
#'   necessary to run the model. 
#' @param base_dir directory in which \code{ref-file} will be found.
#' @return  an unevaluated models object
#' @param N.prob,cost,effect,init arguments to \code{\link{run_models}}
#' @param base_model which of the models should be the base model
#' @param run_demo,run_psa Useful for limiting computation, for example, 
#'   when running a model interactively in shiny.
#' @param ... additional arguments
#' @details
#' 
#'   Relies on the inputs collated by \code{\link{gather_model_info}}.
#'   These pieces were broken apart to facilitate putting together
#'   a shiny interface.
#' 
#'      For more information see the vignette: 
#'      \code{vignette("file-input", package = "heemod")}.`
...
#' \item \code{model_runs} (always)
#'   \item \code{dsa} (deterministic sensitivity
#'   analysis) - if appropriate parameters provided 
#'   \item \code{psa} (probabilistic sensitivity analysis) - 
#'     if appropriate parameters provided
#'    \item \code{demographics} results across different demographic groups
#'    }
#'
...
#' Read in models specified by files
#'
#' @param base_dir directory in which \code{ref-file} will be found
#' @param ref_file name of a file specifying where model inputs will be found.
#' @param base_model which of the models should be the base model
#' @details The reference file should have two columns, 
#'   \code{data} and \code{file}.
#'   The \code{data} column must include entries
#'   \code{state}, \code{tm}, and \code{parameters}, with the corresponding
#'   entries in \code{file} naming the files
#'   containing information on states, the transition matrix,
#'   and parameters.  The parser  also
#'   looks for \code{demographics}, for a file containing demographic
#'   data.
#'   
#'      For more information see the vignette: 
#'      \code{vignette("file-input", package = "heemod")}.`
...
#'
#' 
import_models_from_files <- function(base_dir, ref_file, base_model){
  
  ref <- f_read_file(file.path(base_dir, ref_file))
  ref$file <- file.path(base_dir, ref$file)
  
  state_info <- f_parse_multi_spec(ref[ref$data == "state", "file"],
                                   split_on = ".model",
                                   group_vars = "state")
  
  tm_info <- f_parse_multi_spec(ref[ref$data == "tm", "file"],
                                split_on = ".model",
  ...
  models <- models[c(which_base, setdiff(1:length(models), which_base))]
